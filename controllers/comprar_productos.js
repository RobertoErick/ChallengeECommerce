...
const crearNuevoProducto = (imagen, categoria, nombre, precio, id, userId) => {
    ...
    if (contenedorMapping.hasOwnProperty(categoria)) {
        ...
        const contenido = `
            ...
            <button id="${id}" class="comprar__boton" data-btn>Comprar</button>
        `;
        ...
        btn.addEventListener("click", () => {
            ...
            // Obtener la información del producto según el id (puedes usar tu servicio existente)
            clientServices.detalleCliente(id)
                .then((producto) => {
                    // Añadir el producto al carrito
                    clientServices.agregarProductoAlCarrito(producto, userId)
                        .then(() => {
                            alert(`Producto ${producto.nombre} agregado al carrito.`)
                        })
                        .catch((error) => console.error(`Error al agregar al carrito: ${error.message}`));
                })
                .catch((error) => console.error(`Error al obtener detalles del producto: ${error.message}`));
        });
...
clientServices.listaClientes()
    .then((data) => {
        ...
        data.forEach(({ imagen, categoria, nombre, precio, id }) => {
            crearNuevoProducto(imagen, categoria, nombre, precio, id, userId);
        });
    })
